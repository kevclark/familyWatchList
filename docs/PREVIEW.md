...
| Phone's **Wireless debugging** toggle disables itself within seconds of reconnecting (2026-08-23, Kev, unresolved) | Not root-caused yet — no confirmed timeout in stock Android, so likely OEM-specific (aggressive battery/security management killing the service) or Wi-Fi instability (band-switching, congestion) rather than a documented "feature". Re-enabling and reconnecting (new port each time — see the pairing-survives-reboots note above) is the only known workaround so far. Worth checking, next time it happens: does it correlate with the screen locking/sleeping, or with a specific app/battery-saver mode? If a pattern emerges, note it here. |
